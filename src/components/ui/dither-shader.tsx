...
interface DitherShaderProps {
  ...
  /** Object position (CSS-like, e.g. "50% 50%" for center, "65% center") */
...
function parsePosition(pos: string): [number, number] {
  ...
  const getPct = (s: string): number => {
    if (s.includes("%")) return parseFloat(s) / 100;
    if (s === "left" || s === "top") return 0;
    if (s === "right" || s === "bottom") return 1;
    return 0.5;
  };
  const x = parts[0] ? getPct(parts[0]) : 0.5;
  const y = parts[1] ? getPct(parts[1]) : 0.5;
  return [x, y];
}
  ...
  }, [src, dimensions, objectFit, objectPosition, animated, animationSpeed, applyDithering]);
